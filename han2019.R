...
basic_url<-"https://search.naver.com/search.naver?where=news&sm=tab_pge&query=%EB%B6%80%EB%8F%99%EC%82%B0&sort=0&photo=3&field=0&pd=3&ds=2019.01.01&de=2019.12.31&cluster_rank=10&mynews=1&office_type=2&office_section_code=14&news_office_checked=1028&nso=so:r,p:from20190101to20191231,a:all&start="
urls<-NULL
for(index in 0:19){
...
body<-str_replace_all(body_raw,"\n","") %>% str_replace_all("\t","") %>% 
  ...
  str_replace_all("\\(\\)","") %>% 
  str_replace_all("동영상 뉴스 영상한겨레 정기구독생방송 한겨레 라이브한겨레신문  무단전재 및 재배포 금지","") %>%
  str_replace_all("페북에서 한겨레와 만나요신문 보는 당신은 핵인싸 분이면 뉴스 끝  분컷한겨레신문  무단전재 및 재배포 금지","") %>%
  str_replace_all("관련 기사  청와대 총리법무 우선 교체할 듯총리 김진표 법무 추미애 낙점 관련 기사  문재인에게 김진표는","")
...
library(tm) # 영문 텍스트 마이닝
library(KoNLP) # 세종사전 
write.table(body,file="2019_han_house.txt",fileEncoding = "UTF-8")
body_file<-file("2019_han_house.txt",encoding = "UTF-8")
...
myStopwords = c(stopwords('english'),"만나요","한겨레","문제","전체","당신","고기","우리","이번","만나요","핵인싸", "올해","내년","m는","km","kr", "경우","한겨레와",
                "가운데","이날","▶페북에서","co","▶신문","▶분이면","분컷c한겨레신문","
                hani","지난해","사용","이용", "하기","하게","chosun","com","들이","이상","이후","때문")
...
# 단어 구름 시각화 - 별도의 창에 색상, 빈도수, 글꼴, 회전 등의 속성 적용 ㄴ
#x11( ) # 별도의 창을 띄우는 함수

wordcloud(word.df$word, word.df$freq, 
          scale=c(6,1), min.freq=30, random.order=F, 
...
ruleg <- graph.edgelist(rulemat[c(25:126),], directed=F) # [1,]~[11,] "{}" 제외
ruleg
#  edgelist 시각화
#X11()
